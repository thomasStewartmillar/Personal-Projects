...
for dir in */; do
  ...
  if git branch --list main || git branch --list master; then
    ...
    # Go through branches in a repo and rebase off origin master/main respectively
    for branch in $(git branch | grep -v "master" | grep -v "main"); do
      echo "Switching to $branch branch and performing pull --rebase"
      git checkout $branch
   ...
   echo "\n${BLUE}Rebases on $currentRepo done!${RESET}"
